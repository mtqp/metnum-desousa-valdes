...
Para probar esto en detalle, hicimos un experimento elevando el $\alpha$ a 5. Antes de ver los resultados, pensamos un poco lo que sucedería: esto hará que la fórmula completa de la corrección del gradiente esté multiplicada por 5 en nuestro caso. La fórmula era la siguiente:
...
Si aplicamos entonces 500 \% de este factor de corrección, los valores del verde se van a saturar hacia arriba o hacia abajo, ya que si este $\Delta_R(i,j)$ es negativo, entonces restarle 5 veces eso hará más negros los píxeles (más verdes oscuros en todo caso), y si no los resaltará mucho, llevándolos cerca del 255 en el canal verde. Veamos los resultados de PSNR y SSIM antes de ver la imagen.

\vspace{\baselineskip}

Para PSNR en el canal verde, esta imagen consiguió un score de \textbf{24 dB}, algo incluso menor que \textit{Closest Neighbor}. Un detalle para resaltar es que el PSNR para todos los canales es 23.4 dB, por lo tanto si estos valores están tan cercanos quiere decir que el verde, color más importante en nuestro Bayer Array por ser el más importante para el humano, es tan malo como todo el resto de las capas. Si esto sucede, seguramente la imagen se vea muy mal.

\vspace{\baselineskip}

Ahora con SSIM no pasa lo mismo: \textit{¡aún peor!} El valor para todos los canales es de 0.94, mientras que el del canal verde es de 0.85, siendo ambos los valores más bajos que hemos tenido. Esto presume que efectivamente, la imagen se verá mal, y será seguramente la culpa del verde.

Como para terminar de confirmar, veamos la imagen:

\vspace{\baselineskip}

	\begin{center}
		\includegraphics[scale=.3]{../enunciado/images_files/variacion_alfa/alfa5/filtered_img1.png}
		\vspace{2pt}
		\par
		\footnotesize\textit{Resultado de aplicar la corrección con $\alpha = 5$. Se ve claramente que los verdes quedaron saturados, principalmente hacia los verdes más claros pero si se mira detalladamente también hacia los más oscuros en las sombras por ejemplo.}
	\end{center}

\vspace{\baselineskip}

Este ha sido nuestro peor resultado, por lo tanto sabemos que, más allá de 0 y 1, el filtro realmente no sirve. Además, el filtro en su mejor lugar es cuando $\alpha = $0.5 tal como el paper lo indicaba. 

Efectuamos zoom en el loro de la derecha para un detalle de los artifacts:

\vspace{\baselineskip}

	\begin{center}
		\includegraphics[scale=.3]{../enunciado/images_files/cualitativo/parrot_alpha_5.png}
		\vspace{2pt}
		\par
		\footnotesize\textit{Zoom in, Resultado de aplicar la corrección con $\alpha = 5$.}
	\end{center}

\vspace{\baselineskip}

Cualitativamente, vemos que la imagen degradó significativamente su calidad.  Aparecen artifacts particularmente en los cambios abruptos de colores. En el pico del loro se puede ver como aparece \textit{zipping}, el cual no era un artefacto con alfa en valores considerados ``razonables''.

\vspace{\baselineskip}

La imagen, apoyan la evidencia dada por PSNR y SSIM, aumentando su sustento mediante experimentación empírica.

\vspace{\baselineskip}

Salvo que sea a propósito (si se quisiera un efecto como el de la imagen), no consideramos que sobre-saturar alpha sea una buena idea. 
